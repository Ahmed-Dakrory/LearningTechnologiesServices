...
@ManagedBean
@ViewScoped
public class InstructorFeedbackBean {
	...
	public void fillPendingForms()
	{
		...
		if (!authentication.getPrincipal().equals("anonymousUser"))// logged in
		{
			...
			pendingForms=  facade.getPendingByMajorHead(getInsDataFacade.getInsByPersonMail(mail).getId());
			
				//TODO
				//if is not a major head
				//Then get pending by category head id 
				else pendingForms=  facade.getPendingByCategoryHead(getInsDataFacade.getInsByPersonMail(mail).getId());
			}
			catch(Exception ex)
			{
		  ...
		  archievedForms=new ArrayList<FeedbackDTO>();
			...
			if (!authentication.getPrincipal().equals("anonymousUser"))// logged in
			{
				...
				{try{
					//Check if the logged in instructor is major head
					if(majorFacade.isMajorHead(getInsDataFacade.getInsByPersonMail(mail).getId()))
						archievedForms=  facade.getOldByMajorHead(getInsDataFacade.getInsByPersonMail(mail).getId());
					//TODO
					//if is not a major head
					//Then get pending by category head id 
					else archievedForms=  facade.getOldByCategoryHead(getInsDataFacade.getInsByPersonMail(mail).getId());
				}
				catch(Exception ex)
				{
					JavaScriptMessagesHandler.RegisterErrorMessage(null, "Error In getting feedbacks");
	  ...
	  public void showDetails(FeedbackDTO form) 
	    {
	    	try {
	    		HttpServletRequest origRequest = (HttpServletRequest)FacesContext.getCurrentInstance().getExternalContext().getRequest();
	    		StringBuffer url=origRequest.getRequestURL();
	    		if(url.toString().contains("feedbackOldIns"))
	    		{
	    			FacesContext.getCurrentInstance().getExternalContext().redirect
					("detailedFeedback.xhtml?id="+form.getId()+"&cases=Ins&oldMood=1");
	    		}
	    		else 
				FacesContext.getCurrentInstance().getExternalContext().redirect
				("detailedFeedback.xhtml?id="+form.getId()+"&cases=Ins");
			} catch (IOException e) {
