 ...
 *
 */
@ManagedBean(name = "InstructorTAEvalSubmission",eager=false)
@SessionScoped
public class InstructorTAEvalSubmission {
	...
	public void startSurvey() {
		try {
			...
			for (int i = 0; i < selectedTAsLst.size(); i++) {
				...
				currentQues = facade
						.getBySectionID(QuestionsCategory.Ins_Eval_TA.getID());
			...
			FacesContext.getCurrentInstance().getExternalContext()
					.redirect("insEval.xhtml?-faces-redirect=true");
			}
		} catch (Exception ex) {
			ex.printStackTrace();
...
public void switchToQuestions(Integer insID){
	...
	public void endSurvey() {
		...
		for (int i = 0; i < instructorsQuestions.get(j).size(); i++) {
    		...
    		init();
    		FacesContext.getCurrentInstance().getExternalContext().redirect("insEvalSettings.xhtml?-faces-redirect=true");
