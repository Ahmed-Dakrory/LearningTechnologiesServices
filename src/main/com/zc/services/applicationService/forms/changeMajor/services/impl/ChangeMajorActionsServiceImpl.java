 ...
 */
@Service
public class ChangeMajorActionsServiceImpl implements IChangeMajorActionsService{
	...
	public ChangeMajorDTO getByID(Integer id) {
		...
		try{
			...
			List<PetitionsActions> actions=actionRep.getByPetitionIDAndForm(form.getId(),FormTypesEnum.CHANGEMAJOR.getValue());
	...
	public void addComment(PetitionsActionsDTO dto, Integer instructorID) {
		try
		{
			...
			if(dto.getId()==null)
			{
				...
				action.setFormType(FormTypesEnum.CHANGEMAJOR);
...
try{
			...
			ChangeMajorForm oldForm=rep.getById(dto.getId());
