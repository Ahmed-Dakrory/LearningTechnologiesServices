 ...
 */
@Service
public class JuniorTAActionsServiceImpl implements IJuniorTAActionsService {
	...
	public TAJuniorProgramDTO updateRequest(TAJuniorProgramDTO dto) {
		try{
			TAJuniorProgram entity=assem.toEntity(dto);
			entity=rep.update(entity);
			TAJuniorProgramDTO newDTO=assem.toDTO(entity);
			return newDTO;
		}
		catch(Exception ex){
			ex.printStackTrace();
	...
	@Override
	public boolean approvedBefore(Integer formID, Integer formType,
			 Integer insID) {
		List<PetitionsActions> actions=actionRep.getByPetitionIDAndFormAndIns(formID, formType, insID);
		boolean result=false;
		for(int i=0;i<actions.size();i++)
		{
			if(actions.get(i).getActionType().equals(PetitionActionTypeEnum.Approved))
			{
				result=true;
				break;
			}
