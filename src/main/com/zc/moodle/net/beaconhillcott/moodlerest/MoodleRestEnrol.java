...
public class MoodleRestEnrol {
  ...
  //  private static final int BUFFER_MAX=4000;
    ...
    public static MoodleCourseUser[] getEnrolledUsers(long courseid, String withcapability, long groupid, boolean onlyactive) throws MoodleRestEnrolException , UnsupportedEncodingException, MoodleRestException {
        ...
        if (courseid<0) throw new MoodleRestEnrolException(); else data.append("&").append(URLEncoder.encode("courseid", MoodleServices.ENCODING)).append("=").append(URLEncoder.encode(""+courseid, MoodleServices.ENCODING));
        ...
        if (groupid<0) throw new MoodleRestEnrolException(); else data.append("&").append(URLEncoder.encode("groupid", MoodleServices.ENCODING)).append("=").append(URLEncoder.encode(""+groupid, MoodleServices.ENCODING));
    ...
    public static void enrolUsers(MoodleEnrolUser[] user) throws UnsupportedEncodingException, MoodleRestEnrolException, MoodleRestException {
        try {
            ...
            for (int i=0;i<user.length;i++) {
                ...
                if (user[i].getRoleId()==-1) throw new MoodleRestEnrolException(); else data.append("&").append(URLEncoder.encode("assignments["+i+"][roleid]", MoodleServices.ENCODING)).append("=").append(URLEncoder.encode(""+user[i].getRoleId(), MoodleServices.ENCODING));
                if (user[i].getUserId()==-1) throw new MoodleRestEnrolException(); else data.append("&").append(URLEncoder.encode("assignments["+i+"][userid]", MoodleServices.ENCODING)).append("=").append(URLEncoder.encode(""+user[i].getUserId(), MoodleServices.ENCODING));
                if (user[i].getContextId()==-1) throw new MoodleRestEnrolException(); else data.append("&").append(URLEncoder.encode("assignments["+i+"][contextid]", MoodleServices.ENCODING)).append("=").append(URLEncoder.encode(""+user[i].getContextId(), MoodleServices.ENCODING));
            }
            data.trimToSize();
    ...
    public static void unenrolUsers(MoodleEnrolUser[] user) throws UnsupportedEncodingException, MoodleRestEnrolException, MoodleRestException {
        ...
        try {
            ...
            for (int i=0;i<user.length;i++) {
                if (user[i]==null) throw new MoodleRestEnrolException();
                if (user[i].getRoleId()==-1) throw new MoodleRestEnrolException(); else data.append("&").append(URLEncoder.encode("unassignments["+i+"][roleid]", MoodleServices.ENCODING)).append("=").append(URLEncoder.encode(""+user[i].getRoleId(), MoodleServices.ENCODING));
                if (user[i].getUserId()==-1) throw new MoodleRestEnrolException(); else data.append("&").append(URLEncoder.encode("unassignments["+i+"][userid]", MoodleServices.ENCODING)).append("=").append(URLEncoder.encode(""+user[i].getUserId(), MoodleServices.ENCODING));
                if (user[i].getContextId()==-1) throw new MoodleRestEnrolException(); else data.append("&").append(URLEncoder.encode("unassignments["+i+"][contextid]", MoodleServices.ENCODING)).append("=").append(URLEncoder.encode(""+user[i].getContextId(), MoodleServices.ENCODING));
            }
            data.trimToSize();
            MoodleCallRestWebService.call(data.toString());
         }  catch (IOException ex) {
    ...
    public static MoodleCourse[] getUsersCourses(long userId) throws UnsupportedEncodingException, MoodleRestException {
      ...
      if (MoodleCallRestWebService.getAuth()==null)
        ...
        data.append(MoodleCallRestWebService.getAuth());
      data.append("&").append(URLEncoder.encode("wsfunction", MoodleServices.ENCODING)).append("=").append(URLEncoder.encode(functionCall, MoodleServices.ENCODING));
      if (userId<=0) throw new MoodleRestEnrolException(); else data.append("&").append(URLEncoder.encode("userid", MoodleServices.ENCODING)).append("=").append(URLEncoder.encode(""+userId, MoodleServices.ENCODING));
      NodeList elements=MoodleCallRestWebService.call(data.toString());
