...
public class ReportFileGeneration {
	...
	List<instructor_all_survey_ans> allanswersThisYearAndSemesterofins;
	...
	public void generateReport(List<instructor_all_survey_ques>  instructor_all_survey_ques){
		sheet.setColumnWidth(0, 11000);
		sheet.setColumnWidth(1, 7000);
		for (int i=2;i<=(2*instructor_all_survey_ques.size())-5;i++) {

			sheet.setColumnWidth(i, 17000);
		}
	
		
		titleOfSheet(instructor_all_survey_ques);
	}
	
	
	public void titleOfSheet(List<instructor_all_survey_ques>  instructor_all_survey_ques){
	    ...
	    for(int i=0;i<instructor_all_survey_ques.size()-3;i++) {
	    	cell = row.createCell(2*i+2);
		    row.getCell(2*i+2).setCellStyle(style);
		    cell.setCellValue(instructor_all_survey_ques.get(i).getQues());
	   ...
	   for(int i=0;i<allCoursesThresoldResults.size();i++) {
		   ...
		   for(int j=0;j<instructor_all_survey_ques.size()-3;j++) {
